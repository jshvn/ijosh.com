...
- **Dark mode** is automatic via `prefers-color-scheme`, from the dark block in `tokens.css`; `style.css` swaps only the field image (`--field`). `theme-color` is scheme-aware in `head.html`; the mark swaps to its on-dark file via `<picture>`. No toggle/JS.
...
- **The lattice** is the mark's grid: 20px cells on a 24px pitch below 1100px, doubled to 40 / 48 above. The field is positioned at the card's top-left corner, and the card's size is a cell plus whole pitches, so its edges land between cells. Wide: 1000 × 568, photo pane 400px. Narrow: as wide as the screen allows in whole pitches (up to 596px), running to the bottom of the page as a sheet; the photo takes any extra height.
